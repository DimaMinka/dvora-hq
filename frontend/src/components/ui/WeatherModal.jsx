...
export default function WeatherModal({ isOpen, onClose, meetingTime }) {
  ...
  const startHour = (meetingTime && typeof meetingTime === 'string' && meetingTime.includes(':'))
      ...
      <div className="relative w-full max-w-md bg-bf-dark/95 border border-bf-cyan/60 clip-hud p-5 flex flex-col gap-4 font-mono text-white">
        ...
        <div className="flex justify-between items-center border-b border-bf-border/30 pb-2">
          <div className="flex flex-col">
            <span className="text-[10px] text-bf-cyan font-bold tracking-widest">// TACTICAL WEATHER BRIEFING</span>
        ...
        <div className="grid grid-cols-3 gap-2 text-center text-[10px]">
          <div className="bg-bf-slate/30 border border-bf-border/20 p-2 rounded">
            <span className="text-slate-400 block text-[8px] uppercase tracking-wider">Thermal Range</span>
            <span className="text-white font-bold text-xs">{minTemp}°C - {maxTemp}°C</span>
          </div>
          <div className="bg-bf-slate/30 border border-bf-border/20 p-2 rounded">
            <span className="text-slate-400 block text-[8px] uppercase tracking-wider">Max Wind</span>
            <span className="text-white font-bold text-xs">{maxWind} km/h</span>
          </div>
          <div className="bg-bf-slate/30 border border-bf-border/20 p-2 rounded">
            <span className="text-slate-400 block text-[8px] uppercase tracking-wider">Precip Risk</span>
